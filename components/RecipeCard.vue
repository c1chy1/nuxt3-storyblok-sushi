...
<template>
  <article class="  rounded-lg overflow-hidden shadow-2xl " :key="uuid">
    <div class="card-hover h-[36rem]   group w-full relative text-right rounded-xl ">
      <div class="card-hover__content after:bg-white before:bg-white dark:after:bg-shrimp-900  dark:before:bg-shrimp-900 dark:bg-shrimp-900 text-base font-display bg-white  flex flex-col items-center h-1/3 ">
        <h3 class="card-hover__title mx-auto w-5/6 font-bold text-2xl font-display text-shrimp-600 dark:text-shrimp-300 w-1/2 z-10">
          {{ content.title }}
        </h3>
        <p class="card-hover__text z-10"> {{ content.description }}</p>

        <img
            class=" mt-6 px-auto  dark:text-red dark:contrast-[1] group-hover:-translate-x-[8rem] group-hover:scale-75 w-16 h-16 grayscale contrast-[.25] z-10 transform transition-all  duration-500 "
