<template>
  <div class="content h-72 fixed left-12 bottom-0">

    <button class="shareButton main bg-[#FA6A14] dark:bg-dark-navigation"
            :class="[{ open : show  } , check ? '' : 'sent']"
            @click="open">

      <svg class="share dark:fill-white" style="width:24px;height:24px" viewBox="0 0 24 24">
        <path
            d="M18,16.08C17.24,16.08 16.56,16.38 16.04,16.85L8.91,12.7C8.96,12.47 9,12.24 9,12C9,11.76 8.96,11.53 8.91,11.3L15.96,7.19C16.5,7.69 17.21,8 18,8A3,3 0 0,0 21,5A3,3 0 0,0 18,2A3,3 0 0,0 15,5C15,5.24 15.04,5.47 15.09,5.7L8.04,9.81C7.5,9.31 6.79,9 6,9A3,3 0 0,0 3,12A3,3 0 0,0 6,15C6.79,15 7.5,14.69 8.04,14.19L15.16,18.34C15.11,18.55 15.08,18.77 15.08,19C15.08,20.61 16.39,21.91 18,21.91C19.61,21.91 20.92,20.61 20.92,19A2.92,2.92 0 0,0 18,16.08Z"/>
      </svg>
      <svg class="check dark:fill-white" style="width:24px;height:24px" viewBox="0 0 24 24">
        <path d="M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z"/>
      </svg>
      <svg class="close dark:fill-white" style="width:24px;height:24px" viewBox="0 0 24 24">
        <path
            d="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z"/>
      </svg>
    </button>
    <button class="shareButton fb bg-[#FA6A14] dark:bg-dark-navigation" @click="close" :class="{open : show}">
      <Icon name="ri:facebook-fill"/>
    </button>
    <button class="shareButton ig bg-[#FA6A14] dark:bg-dark-navigation" @click="close" :class="{open : show}">
      <Icon name="icon-park-solid:instagram"/>
    </button>
    <button class="shareButton tw  bg-[#FA6A14] dark:bg-dark-navigation" @click="close" :class="{open : show}">
      <Icon name="prime:twitter"/>
    </button>

  </div>
</template>
<script setup lang="ts">
const show = ref();
const check = ref(true);

const open = () => {


  show.value = !show.value
  check.value = true
}

const close = () => {

  show.value = false
  check.value = !check.value

}

</script>

<style scoped lang="sass">
@import url("https://fonts.googleapis.com/css?family=Roboto:400,400i,700")

body

  cursor: url(data:image/png;base64,), auto

.content


  display: flex
  justify-content: center
  align-items: flex-start
  flex-direction: column
  border-radius: 5px

  .shareButton.main
    .share, .close, .check
      position: absolute
      top: 1rem
      left: 1rem
      transition: all 150ms

    .share, &.open .close, &.sent .check
      transform: rotate(0) scale(1) !important
      opacity: 1

    .close, &.open .share, .check, &.sent .share
      opacity: 0
      transform: rotate(90deg) scale(0)

.shareButton, .shareButton.open
  border: none
  border-radius: 50%
  padding: 1rem
  overflow: hidden
  outline: none
  margin: 0.5rem
  width: 24px
  height: 24px
  box-sizing: content-box
  transition: all 200ms
  position: relative
  opacity: 1
  transform: scale(1)
  box-shadow: 0 0 0 rgba(0, 0, 0, 0)
  cursor: url(data:image/png;base64,), auto

  &:hover
    transform: scale(1.1) translateY(-3px)
    box-shadow: 0 3px 6px rgba(0, 0, 0, 0.25)

  &:active
    cursor: url(data:image/png;base64,), auto

  svg
    display: block
    width: 24px
    height: 24px
    opacity: 1
    transition: all 150ms
    transform: scale(1) !important

.fb, .shareButton.open.ig
  transition-delay: 100ms

.tw, .shareButton.open.tw
  transition-delay: 50ms

.ig, .shareButton.open.fb
  transition-delay: 0ms

.fb, .tw, .ig
  width: 0
  height: 0
  overflow: hidden
  padding: 0
  margin: 0
  opacity: 0
  transform: scale(0)

  svg
    width: 0
    height: 0
    opacity: 0
    transform: scale(0)
</style>
